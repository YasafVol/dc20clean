...
const Attacks: React.FC<AttacksProps> = ({
  ...
  const calculateAttackData = (weapon: WeaponData): AttackData => {
    ...
    const damageString = weapon.versatileDamage ? `${weapon.damage}(${weapon.versatileDamage})` : weapon.damage.toString();
  ...
  return (
    <StyledAttacksSection>
      <StyledAttacksHeader>
        ...
        <StyledAddWeaponButton onClick={addWeaponSlot}>
          + Add Weapon
        </StyledAddWeaponButton>
      </StyledAttacksHeader>
      ...
      <StyledAttacksContainer>
        <StyledAttacksHeaderRow>
          ...
          <StyledHeaderColumn align="center">
            <StyledInfoIcon title="Damage calculation info">
              i
            </StyledInfoIcon>
          </StyledHeaderColumn>
        </StyledAttacksHeaderRow>
          ...
          attacks.map((attack, index) => {
            ...
            return (
              <StyledAttackRow key={attack.id}>
                ...
                {/* Base Damage */}
                <StyledDamageCell
                  title={weapon ? `Base weapon damage: ${weapon.damage}${weapon.versatileDamage ? ` (${weapon.versatileDamage} when two-handed)` : ''}` : ''}
                >
                  {weapon ? (weapon.versatileDamage ? `${weapon.damage}(${weapon.versatileDamage})` : weapon.damage) : '-'}
                </StyledDamageCell>
                
                {/* Heavy Damage */}
                <StyledDamageCell
                  color="#d2691e"
                  title={weapon ? (weapon.properties.includes('Impact') 
                ...
                {/* Damage Type */}
                <StyledDamageTypeCell
                  title={weapon ? `${weapon.damageType} damage` : ''}
                >
                  {weapon ? (
                    weapon.damageType === 'slashing' ? 'S' : 
                    weapon.damageType === 'piercing' ? 'P' : 
                    weapon.damageType === 'bludgeoning' ? 'B' : 'X'
                  ) : '-'}
                </StyledDamageTypeCell>
                ...
                <div style={{ textAlign: 'center', fontSize: '1.1rem' }}>
                  {weapon ? (
                    <StyledInfoIcon title={getDamageCalculationTooltip(weapon)}>
                      i
                    </StyledInfoIcon>
                  ) : '-'}
                </div>
