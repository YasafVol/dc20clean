...
test('10 - maneuvers add and delete', async ({ page }) => {
	...
	if ((await maneuversHeading.count()) === 0) {
		console.log(
			'Maneuvers section not available for this class/character — skipping maneuver add/delete test'
		);
	...
	const addManBtn = await failIfMissing(
		page,
		'[data-testid="add-maneuver"], role=button[name=/add maneuver|add manoeuvre/i], button:has-text("Add Maneuver")',
		'e2e/screenshots/10-03-man-missing.png',
		'add maneuver button'
	);
	await addManBtn.click().catch(() => {});
	const mName = await failIfMissing(
		page,
		'[data-testid="maneuver-name"], input[placeholder="Name"]',
		'e2e/screenshots/10-04-man-name-missing.png',
		'maneuver name input'
	);
	await mName.fill('Test Man').catch(() => {});
	await page
		.getByRole('button', { name: /save maneuver|save manoeuvre/i })
		.click()
		.catch(() => {});
	await expect(page.getByText(/Test Man/i))
	...
	if ((await delMan.count()) > 0) {
		await delMan.click().catch(() => {});
		await page.screenshot({ path: 'e2e/screenshots/10-06-man-deleted.png', fullPage: true });
		console.log('deleted a maneuver');
	}
});
