...
3.  **New Conditional Stage: "Leveling Choices"**
    ...
    *   It will serve as a central hub for spending all accumulated Talents and Path Points.
...
*   **Action:** All `..._table.json` files have been converted into type-safe TypeScript modules in `src/lib/rulesdata/classes-data/progression/`.
*   **Schema Change:** The ambiguous `features: string` property has been replaced with a structured `gains: LevelGains` object.
*   **`LevelGains` Interface (`class.schema.ts`):**
    ```typescript
...
**Layout:** A single-column layout divided into two primary sections.

**Section 1: Talent Selection**

*   **Header:** Displays a counter: **Talents to Choose: [X]**
*   **Accordions:**
    1.  **General Talents:** Lists universal options.
    2.  **[ClassName] Talents:** Dynamically displays class-specific talents, greying out those with unmet prerequisites.
    3.  **Multiclass Talents:** Uses a nested accordion system to select a class, then a feature.

**Section 2: Path Point Allocation**

*   **Header:** Displays a counter: **Path Points to Spend: [Y]**
*   **Content:** Two vertical cards for "Martial Path" and "Spellcaster Path".
*   **Interaction:** Each card shows its current point investment, cumulative benefits, and has "+" / "-" buttons.
*   **Note:** The Martial Path card will prominently display the explicit text for the "Spellcaster Stamina" special rule.

**Validation:** The "Next" button is disabled until both counters are at 0.
...
*   The "Leveling Choices" stage appears only when `level > 1` and correctly reflects the number of choices available.
*   All subsequent stages correctly use the aggregated point totals.
*   The final, saved character object is accurate for its level.
