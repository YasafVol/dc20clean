...
> Purpose: Monster designer using DC20's point-buy framework: Statistics Table, role modifiers, tier system, feature point-buy, action builder, seed data, community sharing.
> Owns: Monster statistics, roles, tiers, monster features and actions, monster validation, monster persistence contracts, and monster designer workflows.
> Does not own: Encounter-budget rules, general storage/authentication infrastructure, player-character calculation, or combat execution.
> Authoritative source: `src/lib/rulesdata/dm/monsterStatistics.ts`, `src/lib/rulesdata/dm/monsterRoles.ts`, `src/lib/services/monsterCalculator.ts`, `src/lib/rulesdata/schemas/monster.schema.ts`, `src/routes/dm/monsters/`, and `convex/monsters.ts`.
> Related systems: [Encounters](./ENCOUNTER_SYSTEM_SPEC.MD), [Database](./DATABASE_SYSTEM.MD), [Testing](./TESTING_SYSTEM.MD).
> Status: Active
...
- `/dm/monsters` - Monster Laboratory (list view)
- `/dm/monsters/:id` - Monster Designer (editor)
- `/dm/monsters/new` - Create new monster

**Menu Entry:** "Laboratory" in DM Tools section

**Key Services:**

- `src/lib/services/monsterCalculator.ts` — `calculateMonsterStats()`, `validateMonsterActions()`, `validateFeatureBudget()`, `createDefaultMonster()`, `recalculateMonsterStats()`
- `src/lib/services/monsterImporter.ts` — transforms raw seed JSON into `SavedMonster` schema
- `src/lib/hooks/useMonsterBuilder.tsx` — React hook for monster editor state
...
### 5.4 Manual Stat Adjustments

The designer sidebar lets users adjust combat stats directly with compact +/- controls:

- HP, PD, AD, Attack, and Save DC adjust in whole-number steps.
- Base Damage adjusts in 0.5 steps.
- HP cannot fall below 1. Base Damage cannot fall below 0.
- The adjusted final values are persisted in the saved monster record.

Changing the monster role preserves the user's manual stat deltas. The builder compares the current saved stat values against the previous level/tier/role canonical values, then applies those deltas to the new role's canonical values.

The sidebar reset button restores all combat stats to canonical values for the current level, tier, and role.
...
| `monsters.listOfficial` | `filters?`   | `Monster[]`       | Official monsters (Bestiary)        |
...
- [x] Menu integration: "Laboratory" in DM Tools section
- [x] Vercel + Convex deployment integration

### Phase 9: Import & Seed ✅

- [x] `src/lib/services/monsterImporter.ts` - Raw JSON to SavedMonster transformer
- [x] `src/lib/rulesdata/dm/sampleMonsters.ts` - 8 pre-built sample monsters
- [x] `convex/monsters.ts:seedMonsters` - Bulk seed mutation
- [x] "Seed Sample Monsters" button in MonsterList empty state
- [ ] Export utility (future)

### Sample Monsters (Seed Data)

| Monster         | Level | Tier      | Role        |
| --------------- | ----- | --------- | ----------- |
| Goblin Warrior  | 1     | Standard  | Skirmisher  |
| Skeleton        | 1     | Standard  | Skirmisher  |
| Wolf            | 1     | Standard  | Skirmisher  |
| Orc Berserker   | 2     | Standard  | Brute       |
| Giant Spider    | 2     | Standard  | Controller  |
| Cultist Fanatic | 2     | Standard  | Artillerist |
| Ogre            | 4     | Apex      | Brute       |
| Young Dragon    | 8     | Legendary | Brute       |

Seed files:

- `docs/seed-data/sample-monsters.json` - Raw JSON template
- `src/lib/rulesdata/dm/sampleMonsters.ts` - TypeScript with calculated stats
