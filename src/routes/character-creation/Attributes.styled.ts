...
export const IconButton = styled.button<{ $disabled?: boolean }>`
	...
	border: 1px solid
		${(props) => (props.$disabled ? theme.colors.bg.elevated : theme.colors.crystal.primaryAlpha70)};
	border-radius: 50%;
...
export const InfoBadge = styled.div<{ $type?: 'limit' | 'cap' }>`
	...
	border: 1px solid
		${(props) => (props.$type === 'limit' ? theme.colors.accent.warning : theme.colors.bg.elevated)};
	border-radius: ${theme.spacing[1]};
	color: ${(props) =>
		props.$type === 'limit' ? theme.colors.accent.warning : theme.colors.text.secondary};
	display: flex;
	align-items: center;
	gap: ${theme.spacing[1]};
`;
