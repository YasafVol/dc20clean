...
export function getInventoryItemInfo(item: InventoryItem | null, inventoryData?: InventoryItemData): InventoryItemInfo[] {
  if (!item) return [];
  ...
  switch (item.itemType) {
    case 'Weapon': {
      ...
      if ((item as any).price)
        info.push({ label: 'Price', value: (item as any).price });
      break;
    }
    case 'Armor': {
      info.push({ label: 'Type', value: (item as any).type });
      if ((item as any).pdr)
        info.push({ label: 'PDR', value: (item as any).pdr });
    ...
    case 'Potion': {
      ...
      if ((item as any).price)
        info.push({ label: 'Price', value: `${(item as any).price}g` });
      break;
    }
