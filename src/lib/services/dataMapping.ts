...
export const getAncestryId = (ancestryName: string | null): string | null => {
  ...
  const ancestry = ancestriesData.find((a: IAncestry) => a.name.toLowerCase() === ancestryName.toLowerCase());
...
export const getClassId = (className: string | null): string | null => {
  ...
  const classData = classesData.find((c: IClassDefinition) => c.name.toLowerCase() === className.toLowerCase());
