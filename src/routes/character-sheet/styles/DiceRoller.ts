...
export const StyledDiceRollerContainer = styled.div<{ $isExpanded: boolean }>`
	...
	bottom: 1rem;
	right: 1rem;
	width: ${(props) => (props.$isExpanded ? '380px' : '52px')};
	...
	@media (max-width: 768px) {
		...
		right: 0.5rem;
		width: ${(props) => (props.$isExpanded ? '260px' : '48px')};
...
export const StyledCollapseButton = styled.button<{ $isExpanded: boolean }>`
	...
	border-radius: ${(props) => (props.$isExpanded ? '6px' : '10px')};
	color: var(--crystal-primary-light);
	font-size: ${(props) => (props.$isExpanded ? '1rem' : '1.3rem')};
	cursor: pointer;
	display: flex;
	align-items: center;
