...
interface LanguagesTabProps {
  currentLanguages: Record<string, { fluency: 'basic' | 'advanced' | 'fluent' }>;
  pointsData: BackgroundPointsData;
  conversions: PointConversions;
  actions: ConversionActions;
  onLanguageChange: (languageId: string, fluency: 'basic' | 'advanced' | 'fluent' | null) => void;
}
...
const LanguagesTab: React.FC<LanguagesTabProps> = ({
  ...
  return (
    <StyledTabContent>
      <StyledPointsRemaining>
        Language Points: {pointsData.availableLanguagePoints - pointsData.languagePointsUsed} / {pointsData.availableLanguagePoints} remaining
        {conversions.tradeToLanguageConversions > 0 && (
          <div style={{ 
            fontSize: '0.9rem', 
            color: '#6366f1', 
            ...
            Active conversions: {conversions.tradeToLanguageConversions} trade → {conversions.tradeToLanguageConversions * 2} language
          </div>
        )}
        <div style={{ 
          marginTop: '0.75rem', 
          ...
          <button 
            ...
            style={getButtonStyle(pointsData.availableTradePoints - pointsData.tradePointsUsed >= 1)}
            onMouseEnter={(e) => {
            ...
            <StyledSelectionItem key={language.id}>
              ...
              <StyledLanguageFluency>
                ...
                {(['basic', 'advanced', 'fluent'] as const).map(fluency => {
                  ...
                  const canAfford = isCommon || currentFluency === fluency || pointsData.languagePointsUsed + cost <= pointsData.availableLanguagePoints;
