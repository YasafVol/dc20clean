...
test('07 - weapons add update remove', async ({ page }) => {
  ...
  // Add weapon
  const addWeaponBtn = await failIfMissing(page, '[data-testid="add-weapon"]', 'e2e/screenshots/07-01-weapon-missing.png', 'add weapon button');
  await addWeaponBtn.click();
  
  // Select a weapon from the dropdown
  const weaponSelect = await failIfMissing(page, '[data-testid="weapon-name"]', 'e2e/screenshots/07-02-weapon-select-missing.png', 'weapon select dropdown');
  ...
  await expect(page.getByText('No weapons added. Click "Add Weapon" to add your first weapon.')).toBeVisible();
