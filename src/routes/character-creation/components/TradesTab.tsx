...
const TradesTab: React.FC<TradesTabProps> = ({
  ...
  return (
    <StyledTabContent>
      <StyledPointsRemaining>
        Trade Points: {pointsData.availableTradePoints - pointsData.tradePointsUsed} / {pointsData.availableTradePoints} remaining
        {hasConversions && (
          <div style={{ 
            fontSize: '0.9rem', 
            color: '#6366f1', 
        ...
        <div style={{ 
          ...
          <button 
            ...
            style={getButtonStyle(pointsData.availableTradePoints - pointsData.tradePointsUsed >= 2)}
            onMouseEnter={(e) => {
              ...
              <StyledProficiencySelector>
                {[0, 1, 2, 3, 4, 5].map(level => (
                  <StyledProficiencyButton
                    ...
                    $disabled={level > currentLevel && !canIncreaseProficiency(level - currentLevel, pointsData.tradePointsUsed, pointsData.availableTradePoints)}
                    onClick={() => {
                      if (level <= currentLevel || canIncreaseProficiency(level - currentLevel, pointsData.tradePointsUsed, pointsData.availableTradePoints)) {
