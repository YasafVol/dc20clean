...
vi.mock('../../../components/auth/AuthModeContext', () => ({
	useAppAuth: () => ({ isConvexEnabled: true, isAuthenticated: true, isLoading: false })
}));
...
const mockChar = {
	...
	classId: 'warrior',
	characterState: {
		resources: { current: { currentHP: 10 }, original: { maxHP: 20 } },
		currency: {},
		attacks: [],
		inventory: [],
		defenseNotes: '',
...
describe('CharacterSheetProvider read-only mode', () => {
	it('exposes readOnly=true and saveNow is a no-op when campaignId is provided', async () => {
		...
		const { CharacterSheetProvider, useCharacterSheet } = await import('./CharacterSheetProvider');
