...
test('08 - items add and remove', async ({ page }) => {
	...
	const addItemBtn = await failIfMissing(
		page,
		'[data-testid="add-item"], role=button[name=/add item/i], button:has-text("Add Item")',
		'e2e/screenshots/08-01-item-missing.png',
		'add item button'
	);
	await addItemBtn.click().catch(() => {});
	// Select an item type, then pick the first available item from the name select
	const itemTypeSelect = await failIfMissing(
		page,
		'select[aria-label="Item Type"], [data-testid="item-name"]',
		'e2e/screenshots/08-02-item-name-missing.png',
		'item type/name select'
	);
	...
	try {
		await typeSelect.selectOption('Adventuring Supply');
	} catch {
		/* ignore */
	}
	// Now target the item-name select and pick the first non-empty option
	const itemNameSelect = await failIfMissing(
		page,
	...
	if (process.env.E2E_SCREENSHOTS === '1') {
		try {
			await page.screenshot({ path: 'e2e/screenshots/08-03-item-added.png', fullPage: true });
		} catch (err: unknown) {
	...
	if ((await remItem.count()) > 0) {
		await remItem.click().catch(() => {});
		if (process.env.E2E_SCREENSHOTS === '1') {
			try {
				await page.screenshot({ path: 'e2e/screenshots/08-04-item-removed.png', fullPage: true });
			} catch (err: unknown) {
				console.log('screenshot failed:', String(err));
			}
		}
